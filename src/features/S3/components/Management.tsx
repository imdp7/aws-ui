...
import {Container, Header, SpaceBetween, Button, Link, ButtonDropdown,Table, Box} from '@cloudscape-design/components'
...
const Lifecycle = () => {
  ...
  const items = [
	...
	return (
          ...
          visibleColumns={['name', 'status', 'scope', 'currentVersion', 'nonCurrentVersion', 'expiredObjects', 'multipart']}
          empty={
        ...
        <Header
          ...
          description={
            <>
              Use lifecycle rules to define actions you want Amazon S3 to take during an object's lifetime such as transitioning objects 
              to another storage class, archiving them, or deleting them after a specified period of time.{' '}
...
const Replication = () => {
  ...
  const columnDefinitions = [
	...
	return (
          ...
          columnDefinitions={columnDefinitions}
          loadingText="Loading Resources"
          visibleColumns={['name', 'status', 'destinationBucket', 'destinationRegion', 'priority', 'scope', 'storageClass', 'owner', 'timeControl', 'kmsEncrypted', 'modificationSync']}
          empty={
            <Box textAlign="center" color="inherit">
        ...
        <Header
          ...
          description={
            <>
              Use replication rules to define options you want Amazon S3 to apply during replication such as 
              server-side encryption, replica ownership, transitioning replicas to another storage class, and more.{' '}
              <Link external>Learn More</Link>
            </>
          }
          actions={
...
const Inventory = () => {
  ...
  const columnDefinitions = [
	...
	return (
          ...
          columnDefinitions={columnDefinitions}
          loadingText="Loading Resources"
          visibleColumns={['name', 'status', 'scope', 'destination', 'frequency', 'lastExport', 'format']}
          empty={
            <Box textAlign="center" color="inherit">
              <b>No Replication rules</b>
              <Box padding={{ bottom: 's' }} variant="p" color="inherit">
        ...
        <Header
          ...
          description={
            <>
              You can create inventory configurations on a bucket to generate a flat file list of your objects and metadata. 
              These scheduled reports can include all objects in the bucket or be limited to a shared prefix.{' '}
              <Link external>Learn More</Link>
            </>
          }
          actions={
