...
import {Container, Header, SpaceBetween, Box, ColumnLayout, Link, Button, TagEditor, FormField, Grid, Popover, Table, TextFilter, Alert, Textarea, StatusIndicator, ExpandableSection, Checkbox} from '@cloudscape-design/components'
...
const Overview = () => {
	...
	return (
		<Container
			 header={
				<Header variant="h2">Permissions Overview</Header>
			}>
			
			<SpaceBetween size="xs" direction="vertical">
			<Box fontSize="heading-s" color="text-body-secondary">Access</Box>
		...
		<Container 
			header={
				<Header variant="h2" description={
					<>
					Public access is granted to buckets and objects through access control lists (ACLs), bucket policies, 
					access point policies, or all. In order to ensure that public access to all your S3 buckets and objects is blocked, 
					turn on Block all public access. These settings apply only to this bucket and its access points. 
					AWS recommends that you turn on Block all public access, but before applying any of these settings, ensure that your 
					applications will work correctly without public access. If you require some level of public access to your buckets 
					or objects within, you can customize the individual settings below to suit your specific storage use cases.{" "}
			...
			<SpaceBetween size="xxs">
				<ExpandableSection variant="footer" headerText="Individual Block Public Access settings for this bucket">
				<>
              <div className="dash">
                <Checkbox
	...
	return (
		<Container 
			...
			<Grid gridDefinition={[
				...
				rows={15}
				/>
				<Button disabled iconName="copy">Copy</Button>
			</Grid>
			</Container>
		);
}
...
const Ownership = () => {
	return (
		<Container
			 header={
				<Header variant="h2" info={<Link>Info</Link>}>Object Ownership</Header>
			}>
			
			<SpaceBetween size="xs" direction="vertical">
			<Box fontSize="heading-s" color="text-body-secondary">Object Ownership</Box>
			<Box fontSize="heading-s" color="text-label">Object writer</Box>
			<Box fontSize="body-m" color="text-body-secondary" fontWeight="normal">ACLs are disabled. All objects in this bucket are owned by this account. 
			Access to this bucket and its objects is specified using only policies.</Box>
			</SpaceBetween>
			</Container>
		);
}
...
const ACL = () => {
	...
	return (
		<Container 
			...
			>
			<SpaceBetween size="m">
				<Alert statusIconAriaLabel="info" header="The console displays combined access grants for duplicate grantees">
