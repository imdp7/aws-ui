...
import {Container, Header, SpaceBetween, Box, ColumnLayout, Link, Button, TagEditor, FormField, Grid, Popover, 
Table, TextFilter, Alert, Textarea, StatusIndicator, ExpandableSection, Checkbox, PieChart, MixedLineBarChart, BarChart, LineChart} from '@cloudscape-design/components'
...
const Analysis = () => {
	...
	return (
		<Container header={
			<Header variant="h2" info={<Link>Info</Link>} counter="(0)" 
			description={
				<>
...
const Replication = () => {
	return (
		<Container header={
			...
			description={
				<>
					Monitor the total number and size of objects that are pending replication, and the maximum replication time to the destination Region.{" "}
					<Link external>Learn More</Link>
				</>
			}
			>
		...
		<Alert statusIconAriaLabel="info" header="Replication Time Control not enabled for any replication rules" 
		action={
			<Button>Go to Replication</Button>
		}>
		<>
		To receive replication metrics, choose Go to Replication. When you create replication rules, enable Replication Time Control. 
		<Link external>Learn more </Link>
		</>
		</Alert>
		</Container>
